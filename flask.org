...
- A =block= directive has the same structure as a =simple= directive, but instead of the semicolon it ends with a set of braces ({ and })
...
- HTTP is a block directive in nginx.conf file. It handles HTTP web traffic
  ...
  - *gzip*: directive tells the server to use on-the-fly gzip compression to limit the amount of bandwidth used
    ...
    - If you keep gzip compression enabled then you are trading increased CPU costs in exchange for your lower bandwidth use
...
- The HTTP block of the nginx.conf file contains the simple directive =include /etc/nginx/sites-enabled/*;=
- This allows for ~server~ block configurations to be loaded in from separate files found in the =sites-enabled= sub-directory
- Usually these symlinks to file stored in =/etc/nginx/sites_available/=
- With symlinks we can quickly enable or disable a virtual server while preserving its configuration file
...
- If we defines the access log to a different path in each server block, we can sort the output specific to each virtual domain into its own file
  e.g
...
- The location setting lets us configure how nginx will respond to requests for resources within the =server=
...
- If we want to change these settings, as well as modify other header fields then use the =proxy_set_header=
...
****** proxy_bind
- proxy_bind is used for when we need to choose a particular source IP address for connecting to a proxied server or an upstream
  e.g
  #+BEGIN_SRC conf
  location /app1/ {
